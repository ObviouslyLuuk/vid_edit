...
def align(video1, video2, dir, time_bin_size=int(44100/5), max_duration=90, plotting=False):
    ...
    time_delay = find_delay_by_peaks(pairs, 1/10, rate)
    if plotting:
        plt.show()
...
def add_a_to_v(video, audio, dir, offset=0, swap_channels=True):
    ...
    command = f"ffmpeg -y -i {dir+video} -i {dir+audio} -c:v copy -map 0:v:0 -map 1:a:0 {swap} {output_path}"
    os.system(command)
...
video1 = "VID_20210820_230500_mp4fix.mp4"
audio2 = "ASMR_Tapping_at_home.wav"
